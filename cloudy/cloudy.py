...
from barak.constants import Ryd, Ryd_Ang, pi
from barak.constants import h as hplanck
from barak.io import saveobj, parse_config, writetable
from cloudy_utils import calc_uvb, calc_local_jnu, read_starburst99


cfg_defaults = dict(abundances=None, table=None, run_cloudy=True,
                    distance_starburst_kpc=None, overwrite=False,
                    logfnu912=None, fesc=None, grains=False,
...
def write_input(outfilename, z, logNHI, metallicity, lognH, fluxfilename=None,
                ...
                iterate='to convergence', grains=False,
                constantpressure=False, cosmicrays=False):
    """ Write an input file for Cloudy.

    By default solar abundances with no grains is are used. The CMB
    ...
    if grains:
        s += ['metals and grains %s log' % metallicity]
    else:
        ...
        for j,nH in enumerate(cfg.lognH):
            for k,Z in enumerate(cfg.logZ):
                ...
                write_input(indir + inname, cfg.z, NHI, Z, nH,
                            ...
                            grains=cfg.grains,
                            constantpressure=cfg.constantpressure,
                            cosmicrays=cfg.cosmicrays)
...
def parse_grid(cfg, outdir='output/'):
    ...
    keys = 'N U Tgas Tstop filename gas_abun dust_abun'.split()
    ...
    for model in models:
        for key in keys:
            if key not in ('N', 'gas_abun', 'dust_abun'):
                try:
    ...
    nZ = len(cfg.logZ)
    for key in keys:
        if key not in ('N', 'Tgas', 'gas_abun', 'dust_abun'):
            ...
            for atom in grid[key]:
                grid[key][atom] = np.array(grid[key][atom]).reshape(nNHI, nnH, nZ, -1)
