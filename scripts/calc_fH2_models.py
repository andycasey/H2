...
nkJRvals =  [
    # SMC like (kappa mult * n mult * R mult / J mult)
    0.2 * 0.1 * 0.1 / 10.,   # 2e-4
    # MW like
    1.* 0.1 * 0.333 / 2.,    # 1.77e-2
    # halo like
    0.04 * 0.05 * 1 / 0.01,  # 0.28
    ]
...
# cm^-2
NH2 = 10**np.arange(10, 22, 0.05)

models = []

for nkJR in nkJRvals:
    fH2 = np.array(approx_fH2(NH2, n*nkJR, JnuLW, kappa, a, T, delta, b), float)
    fH2[fH2 >1] = 1
    Ntot = 2*NH2 / fH2
    models.append([nkJR, NH2, Ntot, fH2])
    # pl.figure(1)
    # #pl.clf()
...
saveobj('models.sav', models, overwrite=1)
